...
export default function ViewSalesReport() {
    ...
    return (
        <div>
            ...
            <div>
                <h1>Filter Sales Report by Day</h1>
            ...
            <div className="filterByDay">
                {filterData && filterData.map((filter) => (
                    filter.map((filter1) => (
                        filter1.items.map((filter2) => (
                        <div key={filter2._id}>
                        <p>{filter1.notes}</p>
                        <p>Medicine Name: {filter2.medicine}, Quantity: {filter2.quantity}</p>
                    </div>
                    ))))))} 
            </div> 

            <div>
                <h1>Filter Sales Report by Medicine Name</h1>
                <label>Enter Medicine Name: </label>
                <input type="text" name="name" />
                <button onClick={handleNameFilter}>Filter</button>

            </div> 
            <div className="filterByName">
                {filterByNameData && filterByNameData.map((filter) => (
                    filter.map((filter1) => (
                        filter1.items.map((filter2) => (
                        <div key={filter2._id}>
                        {filter2.medicine === name ? (
                            <>
                                <p>{filter1.notes}</p>
                                <p>
                                    Medicine Name: {filter2.medicine}, Quantity: {filter2.quantity}
                                </p>
                            </>
                        ) : (
                            <></>
